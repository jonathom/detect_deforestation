...
gf_quarterly_1 <- readRDS("quarterly_iMax1_140_gapfilled.rds")
```
...
bfast_quarter_inf <- bfast_on_tile(gf_quarterly_inf$fill, by = 0.25, ts = 28, order = 2)
bfast_quarter_1 <- bfast_on_tile(gf_quarterly_1$fill, by = 0.25, ts = 28, order = 2)
...
Image(bfast_quarter_inf) + ggtitle("Quarterly Data") + theme(plot.title = element_text(size=22))
Image(prodes) + ggtitle("PRODES Data") + theme(plot.title = element_text(size=22))

table1
table2

array(c(accuracies(table1), accuracies(table2), accuracies(table3), accuracies(table4)), dim = c(6,4), dimnames = list(c("Overall Accuracy", "Prod. Acc. FALSE", "Prod. Acc. TRUE", "User's Acc. FALSE", "User's Acc. TRUE", "Kappa"), c("monthly", "quarterly", "quarter iMax=inf", "quarter iMax=1")))
```
